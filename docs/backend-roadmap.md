...
| R10 | **A cron-driven queue has a floor on latency and no liveness signal.** Work waits up to one cron interval, and a cron that stops firing looks exactly like a quiet queue | Medium — §27 | Jobs carry `run_after`, so a delayed run is late rather than wrong. The runner records each pass, so "when did this last run?" is answerable; alerting on that gap belongs with M8's observability |
...
| R11 | **Tacit renewal without the legally required prior notice.** Renewing a committed subscription silently is the kind of clause consumer law constrains, and the constraint is a deadline — a notice sent late is a notice not sent | High, regulatory | **Still open, and the gate has moved.** M5.1 shipped the dates the notice is derived from (`term_ends_at`, `commitment_ends_at`, `notice_days`) and M7.1 shipped the channel that keeps the attempt *and* the rendered body, so "did we tell them, and what did we say?" is answerable. What is missing is not mechanism but two things: no job sends the notice yet, and the exact deadlines — especially toward consumers, where they vary by member state — have never been confirmed against official sources. An earlier version of this row said "before M5.1 ships"; M5.1 has shipped, so the gate is now **before any renewal runs unattended in production**. Until then a committed subscription must not auto-renew without a human deciding. Same discipline R3 and R7 impose on e-invoicing dates and VAT rates |
