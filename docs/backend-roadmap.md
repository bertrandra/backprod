...
| R13 | **Deleting a project destroys its history, and nothing says so until it is gone.** `DELETE /projects/{projectId}` is a hard delete and `project_versions` follows through `ON DELETE CASCADE`, so a project with fifty snapshots leaves nothing behind. `restoreProject` restores a project *to* a version and cannot restore a deleted one — the two share a word and not an operation | Medium, data loss | **Closed in U10.** Deletion is a `deleted_at` date now, which is the shape §15 already uses everywhere else in this platform. Nothing is dropped, so no foreign key changed and no orphan was created; the cascade stays for the one deletion that is still real, a tenant being erased. A partial index carries the rule every live read filters on, the bin is a separate list rather than a badge on some rows, and `POST /projects/{projectId}/undelete` puts one back — deliberately not called `restore`, since that already means restoring a project *to a version*. The delete confirmation was reworded: it used to say "removed permanently, this cannot be undone", which was true and is not, and a warning that overstates is one people learn to dismiss. How long a deleted project is *kept* is still open, and belongs to whoever writes the retention sweep with §15 on the other side of it. Originally: **surfaced by U4 rather than by an incident.** The UI roadmap's U4 asked for "restoring a deleted project works, and the deleted state is visible", which turned out to describe an API that does not exist. The frontend ships what is real — a delete that says the snapshots go with it, permanently, with the project's name typed to confirm — and does not fake the rest. The decision to take is whether project deletion should be recoverable at all: a `deleted_at`, a list filter, a contract field and versions that outlive the delete is a small change, but it is a **data-model** change with §15's retention rules on the other side of it, and it should be made deliberately rather than because a screen wanted an undo |
| R14 | **A staff read is traced but not *motivated by the person making it*.** Non-negotiable #21 requires an access across the tenant boundary to be traced, motivated and never silent. What the platform records is who looked, at what, and **under which permission** — `StaffAccess` carries `permission` precisely so the log can answer "on what grounds?". That is the *authority* for the read. It is not the *reason for this particular read*, and no endpoint accepts one: `showTenantForStaff`, `showSupportConversation` and `listAccessLog` take no reason parameter, header or body, and `StaffRoute::permitted()` has nowhere to put one | Medium, governance | **Closed in U10, and the design question was answered by taking both halves.** R14 said a mandatory free-text field *"collects 'support' a thousand times and proves nothing"* while a structured one *"is only as good as the system it points at"* — so a motive is now a `purpose` from a five-value enumeration **and** a `reference` of at least eight characters. The enumeration makes the log countable (how many reads were billing investigations last quarter); the reference makes any single row mean something; a minimum length is enforced by a CHECK so "x" cannot enter the table by any route. It travels as `X-Access-Purpose` and `X-Access-Reason` headers, because these are GETs and the reason must be recorded *with* the access rather than by a second call somebody could skip. **Where it is required is itself the decision:** on the two reads that reveal one tenant's own data — opening a customer, opening a support thread — and on no listing, because listing customers reveals no customer and a platform that demanded a ticket reference to page through a list would teach its staff to type "support" into everything. Reading the audit trail needs none either: making the accountability mechanism the thing people avoid using would defeat it. The console collects it *before* the read through a gate, so nothing is fetched and then apologised for. Originally: **surfaced by U8 rather than by an incident** — the same way U4 surfaced R13. The UI roadmap's U8 asked that "a staff read without a stated reason cannot be performed", which turned out to describe an API that does not exist. The frontend ships what is real and does not fake the rest: every console screen states before the click that the read is recorded and under which permission, the detail confirms it afterwards naming the permission, and the access log is a first-class screen showing the operator their own crossings — an audit nobody can read is an audit nobody is accountable to. A free-text box that went nowhere would have been worse than none: it would have *looked* like the control the non-negotiable asks for. The decision to take is whether a per-read reason belongs in the contract at all. It is a real design question rather than an oversight: a mandatory free-text field collects "support" a thousand times and proves nothing, while a structured one (a ticket reference, a customer request id) is only as good as the system it points at. If it is wanted, it is a request field on the three staff read endpoints, a column on `staff_access_log`, and a rule about what makes a reason acceptable — small in code, and a governance decision before it is a schema change |
