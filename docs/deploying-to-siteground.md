...
`payment_intent.canceled`, `refund.updated` and `charge.dispute.created`; its
signing secret is the `whsec_…`. Every other event is accepted and ignored, so
subscribing to more costs nothing but traffic.
