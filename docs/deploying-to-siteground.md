...
| `extension "btree_gist" is not available` during a migration (from `composer run migrate`, or `setup.php`) | The server's PostgreSQL was built without the `contrib` modules, so `btree_gist` is not installed *on the server* — a different problem from a permission error, and not one the account's own database user can fix with `CREATE EXTENSION` (it is a trusted extension since PostgreSQL 13, but that only helps once the module exists to be created). Two tables' data integrity depends on it: `tax_rates` refuses two overlapping windows for the same country and rate kind, and `offer_versions` refuses two ACTIVE versions of the same offer at once — both `EXCLUDE USING gist` constraints. Ask SiteGround support to install `btree_gist` (a standard, lightweight PostgreSQL contrib module — most managed hosts ship it, so this is usually a quick fix) and re-run. `setup.php` is safe to resubmit: it already told you `.env` is written and already-applied migrations will not run twice, and that is Doctrine's own migration metadata table, not a guess |
