...
| area | for |
|---|---|
| `tenant.subscription` | **whose answer this is, first** (2026-09-26): the person asking and the organisation they are asking in, read from the session and from `/tenants/current` — a seat is bought by one person and given up by them alone, and somebody belonging to two organisations is one switcher click from reading the other one's subscription with nothing on the page to say so. The organisation is left out rather than guessed where that read is refused. Then the organisation's subscription, its schedule, entitlements, changing offer, cancelling, resuming — those controls with `billing.manage`; and the caller's own seat (§13.1), shown to its holder beside it and given up by them alone. **People** (2026-09-19): who a subscription covers and how many it may — the offer's `users` quota, counting the owner; 1 when it sold none — read by anybody, managed by whoever activated it (`owner_user_id`): a member of the organisation from a picker, or anybody by address, who gets an account with no usable password and an invitation link to set one. A member of Ada's seat is entitled by it |
| `billing.invoices` | invoices, the PDF (ADR-035), issuing, cancelling, crediting. **Every row says whose it is** (2026-09-26): `billing.manage` shows an administrator every invoice the organisation has, and until then the rows carried an amount, a status and a number and nothing that told one colleague's from another's. Read from the customer snapshot, so it names who they were when the document was raised (§25) and never who they are now |
| `billing.payments` | payments, refunds, and retrying a failed attempt. **Every row says which document and whose** (2026-09-26): the invoice's legal number in the link — what somebody reconciling against paper is looking for — and the customer it was raised to, from `CollectedPayment`. A draft invoice still reads "the invoice", because a number comes from a gapless sequence at issue and a placeholder is how a hole enters one |
